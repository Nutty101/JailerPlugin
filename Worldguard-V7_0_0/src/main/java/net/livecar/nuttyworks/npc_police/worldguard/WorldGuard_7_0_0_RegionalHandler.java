...
public class WorldGuard_7_0_0_RegionalHandler extends Handler {

    public static final Factory FACTORY = new Factory();
    private static final long MESSAGE_THRESHOLD = 1000 * 2;
    private long lastMessage;

    public WorldGuard_7_0_0_RegionalHandler(Session session) {
        super(session);
    }
        ...
        if (regionFlags.wanted_DenyMin.ordinal() < plrManager.getWantedLevel().ordinal() || regionFlags.wanted_DenyMax.ordinal() > plrManager.getWantedLevel().ordinal()) {
            if (!getSession().getManager().hasBypass(player, (World) from.getExtent()) && moveType.isCancellable()) {
                ...
                if ((now - lastMessage) > MESSAGE_THRESHOLD && message != null && !message.isEmpty()) {
                    plr.sendMessage(WorldGuard_7_0_0.replaceColorMacros(message));
                    lastMessage = now;
                }
    ...
    public static class Factory extends Handler.Factory<WorldGuard_7_0_0_RegionalHandler> {
        @Override
        public WorldGuard_7_0_0_RegionalHandler create(Session session) {
            return new WorldGuard_7_0_0_RegionalHandler(session);
        }
    }
}
